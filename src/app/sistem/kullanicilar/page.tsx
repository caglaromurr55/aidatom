...
export default function KullanicilarPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    // Enforce role checks if needed, but here we view all site managers & lawyers
  ...
  return (
    <>
      <div className="page-header">
        <h1 className="heading-sm">Kullanıcı Yönetimi</h1>
        <p className="text-sm" style={{ color: 'var(--text-secondary)', marginTop: 'var(--space-xs)' }}>
          Sistemdeki site yöneticilerini ve avukatları yönetin, aktif/pasif durumlarını değiştirin.
        </p>
      </div>
      ...
      <div className="page-body">
        ...
        {filteredUsers.length === 0 ? (
          <div className="empty-state">
            ...
            <p>Seçtiğiniz filtrelere uygun kullanıcı kaydı bulunmamaktadır.</p>
          </div>
        ) : (
                  ...
                  <tr key={user.id}>
                    ...
                    <td>
                      <span className="badge badge-neutral" style={{ textTransform: 'none' }}>
                        {user.role === 'site_manager' ? 'Site Yöneticisi' : user.role === 'lawyer' ? 'Avukat' : user.role === 'system_admin' ? 'Sistem Admin' : 'Süper Admin'}
                      </span>
                    </td>
                    <td>
